...
При первом использовании Claude Code сам проведёт через OAuth-вход. Либо используйте персональный токен (ниже) — вкладка *Claude Code* в диалоге токена даёт ровно эту команду с уже подставленным токеном:
...
Токен показывается один раз при создании, и в том же диалоге есть готовые сниппеты с уже подставленным токеном: команда **Claude Code**, **JSON-конфиг** для Claude Desktop / Cursor / VS Code / Gemini CLI, вызов **cURL** и пример **Node.js** для REST API — откройте вкладку своего клиента и нажмите «копировать». На той же странице видно время последнего использования, там же токен отзывается. Read-only-токен привязан к команде, в которой выпущен.
