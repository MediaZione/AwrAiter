...
1. Откройте **Settings → Connectors** и добавьте свой коннектор (в некоторых версиях — через **Advanced → Developer mode**).
2. Вставьте `https://awraiter.ai/mcp`, авторизацию оставьте **OAuth**, сохраните.
...
**Инструменты чтения** работают с любым скоупом; помеченные ✍️ меняют данные и требуют OAuth либо токен со скоупом *full*. `deep_analyze` дополнительно тратит AI-кредиты команды — всё остальное бесплатно.
...
| `switch_team` | Переключить активную команду. |
...
| `deep_analyze` | Глубокий AI-анализ канала или сравнение нескольких — считается на серверах AwrAIter, тратит AI-кредиты. |
